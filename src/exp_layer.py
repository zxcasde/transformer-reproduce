...
def create_experiment_dirs(exp_name):
    """创建实验目录结构 - 为每个层数配置创建独立的目录"""
    # 模型保存目录
    model_dirs = {
        'models': f'./save/model/MLM_Layers/{exp_name}',
        'checkpoints': f'./save/model/MLM_Layers/{exp_name}/checkpoints'
    }
    
    # 结果保存目录（合并config、log、plot）
    result_dirs = {
        'results': f'./results/MLM_Layers/{exp_name}'
    }
...
def run_layers_experiment(load_local=False):
    """运行不同Transformer层数的实验"""
    print("="*80)
    print("实验: 不同Transformer层数对比")
    print("="*80)
    ...
    # 不同层数配置
    layer_configs = [
        {'num_layers': 1, 'name': 'layers_1'},
        {'num_layers': 2, 'name': 'layers_2'},
        {'num_layers': 4, 'name': 'layers_4'}
    ]
    ...
    # 绘制对比图
    plot_layers_comparison(all_results)
    
    return all_results

def plot_layers_comparison(results):
    """绘制不同层数的对比图"""
    ...
    for result in results:
        ...
        ax1.plot(epochs, val_loss, label=f'{num_layers} layers', linewidth=2, marker='o')
    
    ax1.set_title('Validation Loss Comparison - Number of Transformer Layers')
    ax1.set_xlabel('Epoch')
    ...
    for result in results:
        ...
        ax2.plot(epochs, val_acc, label=f'{num_layers} layers', linewidth=2, marker='s')
    
    ax2.set_title('Validation Accuracy Comparison - Number of Transformer Layers')
    ax2.set_xlabel('Epoch')
    ...
    # 保存对比图到MLM_Layers总目录
    os.makedirs('./results/MLM_Layers', exist_ok=True)
    plt.savefig('./results/MLM_Layers/layers_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # 打印最终结果对比
    print("\n" + "="*80)
    print("Transformer层数实验最终结果对比")
    print("="*80)
    print(f"{'层数':<8} {'最终验证损失':<15} {'最终验证准确率':<15}")
    print("-" * 50)
    for result in results:
        num_layers = result['config']['num_layers']
        final_loss = result['final_val_loss']
        final_acc = result['final_val_accuracy']
        print(f"{num_layers:<8} {final_loss:<15.4f} {final_acc:<15.4f}")
