...
def create_experiment_dirs(exp_name):
    """创建实验目录结构 - 为每个FFN配置创建独立的目录"""
    # 模型保存目录
    model_dirs = {
        'models': f'./save/model/MLM_FFN/{exp_name}',
        'checkpoints': f'./save/model/MLM_FFN/{exp_name}/checkpoints'
    }
    
    # 结果保存目录（合并config、log、plot）
    result_dirs = {
        'results': f'./results/MLM_FFN/{exp_name}'
    }
...
def run_ffn_experiment(load_local=False):
    """运行不同FFN维度的实验"""
    print("="*80)
    print("实验: 不同FFN维度对比")
    print("="*80)
    ...
    # 不同FFN维度配置
    ffn_configs = [
        {'d_ff': 256, 'name': 'FFN_256'},
        {'d_ff': 512, 'name': 'FFN_512'},
        {'d_ff': 1024, 'name': 'FFN_1024'}
    ]
    ...
    # 绘制对比图
    plot_ffn_comparison(all_results)
    
    return all_results

def plot_ffn_comparison(results):
    """绘制不同FFN维度的对比图"""
    ...
    for result in results:
        ...
        ax1.plot(epochs, val_loss, label=f'FFN={d_ff}', linewidth=2, marker='o')
    
    ax1.set_title('Validation Loss Comparison - FFN Dimension')
    ax1.set_xlabel('Epoch')
    ...
    for result in results:
        ...
        ax2.plot(epochs, val_acc, label=f'FFN={d_ff}', linewidth=2, marker='s')
    
    ax2.set_title('Validation Accuracy Comparison - FFN Dimension')
    ax2.set_xlabel('Epoch')
    ...
    # 保存对比图到MLM_FFN总目录
    os.makedirs('./results/MLM_FFN', exist_ok=True)
    plt.savefig('./results/MLM_FFN/ffn_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    print("FFN维度实验最终结果对比")
    print("="*80)
    print(f"{'FFN维度':<10} {'最终验证损失':<15} {'最终验证准确率':<15}")
    print("-" * 50)
    for result in results:
        d_ff = result['config']['d_ff']
        final_loss = result['final_val_loss']
        final_acc = result['final_val_accuracy']
        print(f"{d_ff:<10} {final_loss:<15.4f} {final_acc:<15.4f}")
