...
from transformer_h_pos import Encoder_Only_MLM 
...
def create_experiment_dirs(exp_name):
    """创建实验目录结构 - 为每个位置编码配置创建独立的目录"""
    # 模型保存目录
    model_dirs = {
        'models': f'./save/model/MLM_POS/{exp_name}',
        'checkpoints': f'./save/model/MLM_POS/{exp_name}/checkpoints'
    }
    
    # 结果保存目录（合并config、log、plot）
    result_dirs = {
        'results': f'./results/MLM_POS/{exp_name}'
    }
...
def run_pos_encoding_experiment(load_local=False):
    """运行位置编码消融实验"""
    print("="*80)
    print("实验: 位置编码消融实验")
    print("="*80)
    ...
    # 不同位置编码配置
    pos_configs = [
        {'pos_encoding_type': 'sinusoidal', 'name': 'pos_sinusoidal'},
        {'pos_encoding_type': 'learned', 'name': 'pos_learned'},
        {'pos_encoding_type': 'relative', 'name': 'pos_relative'},
        {'pos_encoding_type': 'none', 'name': 'pos_none'}
    ]
    ...
    for config in pos_configs:
        ...
        # 创建实验目录 - 使用位置编码配置名称作为子目录
        save_dirs = create_experiment_dirs(config['name'])
        ...
        # 保存配置
        save_training_config(exp_config, save_dirs['results'])
        
        # 创建数据集
        train_dataset = MLMDataset([t for t in train_texts if t and t.strip()], tokenizer, 
                                  max_length=exp_config['max_length'], mlm_prob=exp_config['mlm_prob'])
        ...
        # 创建模型（使用修改后的Encoder_Only_MLM，支持位置编码类型参数）
        model = Encoder_Only_MLM(
            ...
            pos_encoding_type=exp_config['pos_encoding_type'],
            max_len=exp_config['max_length']
        ).to(device)
        
        # 创建优化器和调度器
        optimizer = torch.optim.AdamW(model.parameters(), lr=exp_config['learning_rate'], betas=(0.9, 0.98), eps=1e-9)
        scheduler = TransformerLRScheduler(optimizer, d_model=exp_config['d_model'], warmup_steps=exp_config['warmup_steps'])
        ...
        training_history = train_mlm_with_scheduler(
            ...
            criterion, device, exp_config['epochs'], save_dirs, config['name']
        )
        
        # 绘制训练指标
        plot_mlm_training_metrics(training_history, save_dirs['results'], config['name'])
    ...
    # 绘制对比图
    plot_pos_encoding_comparison(all_results)
    
    return all_results

def plot_pos_encoding_comparison(results):
    """绘制不同位置编码的对比图"""
    ...
    for result in results:
        pos_type = result['config']['pos_encoding_type']
        display_name = pos_names.get(pos_type, pos_type)
        val_loss = result['history']['val_loss']
        epochs = range(1, len(val_loss) + 1)
        ax1.plot(epochs, val_loss, label=display_name, linewidth=2, marker='o')
    
    ax1.set_title('Validation Loss Comparison - Position Encoding Type')
    ax1.set_xlabel('Epoch')
    ...
    for result in results:
        ...
        ax2.plot(epochs, val_acc, label=display_name, linewidth=2, marker='s')
    
    ax2.set_title('Validation Accuracy Comparison - Position Encoding Type')
    ax2.set_xlabel('Epoch')
    ...
    # 保存对比图到MLM_POS总目录
    os.makedirs('./results/MLM_POS', exist_ok=True)
    plt.savefig('./results/MLM_POS/pos_encoding_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # 打印最终结果对比
    print("\n" + "="*80)
    print("位置编码消融实验最终结果对比")
    print("="*80)
    print(f"{'位置编码类型':<15} {'最终验证损失':<15} {'最终验证准确率':<15}")
    print("-" * 50)
    for result in results:
        pos_type = result['config']['pos_encoding_type']
        display_name = pos_names.get(pos_type, pos_type)
        ...
        print(f"{display_name:<15} {final_loss:<15.4f} {final_acc:<15.4f}")
...
if __name__ == '__main__':
    set_seed()
    results = run_pos_encoding_experiment()