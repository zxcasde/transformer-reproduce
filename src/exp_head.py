...
def create_experiment_dirs(exp_name):
    """创建实验目录结构 - 为每个注意力头数配置创建独立的目录"""
    # 模型保存目录
    model_dirs = {
        'models': f'./save/model/MLM_Heads/{exp_name}',
        'checkpoints': f'./save/model/MLM_Heads/{exp_name}/checkpoints'
    }
    
    # 结果保存目录（合并config、log、plot）
    result_dirs = {
        'results': f'./results/MLM_Heads/{exp_name}'
    }
...
def run_heads_experiment(load_local=False):
    """运行不同注意力头数的实验"""
    print("="*80)
    print("实验: 不同注意力头数对比")
    print("="*80)
    ...
    # 不同头数配置
    head_configs = [
        {'n_heads': 2, 'name': 'heads_2'},
        {'n_heads': 4, 'name': 'heads_4'},
        {'n_heads': 8, 'name': 'heads_8'}
    ]
    ...
    # 绘制对比图
    plot_heads_comparison(all_results)
    
    return all_results

def plot_heads_comparison(results):
    """绘制不同头数的对比图"""
    ...
    for result in results:
        ...
        ax1.plot(epochs, val_loss, label=f'{n_heads} heads', linewidth=2, marker='o')
    
    ax1.set_title('Validation Loss Comparison - Number of Attention Heads')
    ax1.set_xlabel('Epoch')
    ...
    for result in results:
        ...
        ax2.plot(epochs, val_acc, label=f'{n_heads} heads', linewidth=2, marker='s')
    
    ax2.set_title('Validation Accuracy Comparison - Number of Attention Heads')
    ax2.set_xlabel('Epoch')
    ...
    # 保存对比图到MLM_Heads总目录
    os.makedirs('./results/MLM_Heads', exist_ok=True)
    plt.savefig('./results/MLM_Heads/heads_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    print("注意力头数实验最终结果对比")
    print("="*80)
    print(f"{'头数':<8} {'最终验证损失':<15} {'最终验证准确率':<15}")
    print("-" * 50)
    for result in results:
        n_heads = result['config']['n_heads']
        final_loss = result['final_val_loss']
        final_acc = result['final_val_accuracy']
        print(f"{n_heads:<8} {final_loss:<15.4f} {final_acc:<15.4f}")
